...
open class ApiClient(val baseUrl: String) {
    ...
    protected fun updateAuthParams(requestConfig: RequestConfig) {
        if (requestConfig.headers["X-API-Token"].isNullOrEmpty()) {
            if (apiKey["X-API-Token"] != null) {
                if (apiKeyPrefix["X-API-Token"] != null) {
                    requestConfig.headers["X-API-Token"] = apiKeyPrefix["X-API-Token"]!! + " " + apiKey["X-API-Token"]!!
                } else {
                    requestConfig.headers["X-API-Token"] = apiKey["X-API-Token"]!!
                }
            }
        }
    }
